...
@section('content')
    ...
    <div class="flex justify-between items-center mb-6">
        <h1 class="text-amber-900 text-2xl font-bold">One-Time Tokens</h1>   
        <a href="{{ route('one-time-tokens.create') }}" class="px-4 py-2 bg-blue-500 text-amber-900 rounded">Create</a>
    </div>
    ...
    <div class="flex flex-col border border-white rounded">
        @foreach ($oneTimeTokens as $oneTimeToken)
            <a href="{{ route('one-time-tokens.show', $oneTimeToken) }}" class="border-b border-white p-4 flex justify-between items-center">
                <p class="text-amber-900 text-lg font-bold">{{ $oneTimeToken->user->name }}</p>
