        ...
        <label class="flex flex-col text-white">
            ...
            <select name="fixture_id" class="w-full p-2 text-white bg-black border border-white">
                ...
                @foreach ($fixtures as $fixture)
                    <option value="{{ $fixture->id }}" {{ old('fixture_id') == $fixture->id ? 'selected' : '' }}>{{ $fixture->team1->long_name }} vs {{ $fixture->team2->long_name }} - {{ $fixture->started_at->format('Y-m-d H:i') }}</option>
                @endforeach
            </select>
        </label>
        @error('fixture_id')
