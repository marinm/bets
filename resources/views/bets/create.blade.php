...
@section('content')
    ...
    <h1 class="text-amber-900 text-2xl font-bold mb-6">Create a Bet</h1>
    ...
    <form action="{{ route('bets.store') }}" method="POST">
        ...
        @error('winner_team_id')
            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
        @enderror
        <button type="submit" class="w-full mt-4 px-4 py-2 bg-blue-500 text-amber-900 rounded">Create Bet</button>
    </form>
@endsection
